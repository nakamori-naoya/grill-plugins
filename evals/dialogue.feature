...
Scenario: 7問目以降は問わず推奨付きの未決として返す
  Given 入力の問いと調査で見つけた論点を合わせて8件ある
  And 問い手は成果を左右する順に6件を選んでいる
  When 6件目の回答を受け取り、問い手が次の問いを提示しようとする
  Then 7件目と8件目は利用者へ提示されない
  And 7件目と8件目は推奨回答と理由を添えたopenの未決として一覧に載る
  And 利用者は6件の決定と2件の未決を含む一覧へ明示合意を求められる
# NOTE:
#   Rule: 1回の対話で問うのは最大6問で、残りは推奨を仮置きした未決にする
#   Reason: 問い詰めが長引くと成果物が出ず、利用者は成果物を見てからの方が残る論点を判断しやすいため
