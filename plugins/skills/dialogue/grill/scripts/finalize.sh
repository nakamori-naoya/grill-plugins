...
jq -e '.version==1 and (.log_dir|type=="string" and length>0)' >/dev/null <<<"$merged" \
  || { echo "[error] version または log_dir が不正" >&2; exit 2; }

log_dir=$(resolve_path "$(jq -r '.log_dir // "decisions"' <<<"$merged")")

out=$(jq -cn --arg q "$PLUGIN_ROOT/references/questioning.md" \
  --arg l "$log_dir" --arg root "$root" --arg pr "$PLUGIN_ROOT" \
  '{contract:1, questioning:$q, log_dir:$l, repo_root:$root, plugin_root:$pr}')
