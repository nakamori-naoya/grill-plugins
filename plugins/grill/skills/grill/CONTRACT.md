...
`references`の省略は追加材料がないことを示す。関連コードや既存文書から調べる責務は残る。プロジェクト固有の規約や文脈は、対象repositoryのAGENTS.md / CLAUDE.mdと`references`で渡される。
...
6問の上限を超えて問わなかった論点は、`state: open`の`open_questions`として返し、`reason`に推奨回答とその理由を書く（例: `上限により未確認。推奨: 出荷日基準（在庫引当の解放時点と揃うため）`）。呼び出し元はこの推奨を仮置きして成果物へ反映できる。
...
- 1回の呼び出しで利用者へ問うのは、入力`questions`と調査で見つけた問いを合わせて最大6問である。成果を左右する順に厳選し、7問目以降は問わず推奨を添えて`open_questions`（`state: open`）へ返す。
- 利用者の回答を待つ。自分の提案を自分で承認しない。
- 決定の理由をその場で確認し、推測で補わない。
- 決められない論点は理由付きで未決にし、取り下げと区別する。
- 厳選した問いの範囲で、決定・未決・取り下げの一覧と対話終了への明示合意を得るまで完了しない。
