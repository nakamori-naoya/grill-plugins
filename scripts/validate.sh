...
for manifest in "$plugin/.codex-plugin/plugin.json" "$plugin/.claude-plugin/plugin.json"; do
  jq -e '.name=="grill" and .version=="0.2.13"' "$manifest" >/dev/null || failed=1
done
jq -e '.name=="grill" and (.plugins|length==1) and .plugins[0].name=="grill" and .plugins[0].version=="0.2.13"' "$ROOT/.agents/plugins/marketplace.json" "$ROOT/.claude-plugin/marketplace.json" >/dev/null || failed=1
...
yq -o=json -I=0 '.' "$plugin/config/defaults.yml" \
  | jq -e '((has("aspects")|not) and (has("prompt_parameters")|not))' >/dev/null || failed=1
